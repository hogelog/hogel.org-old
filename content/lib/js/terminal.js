...
//}
//};
var Terminal = function(win, insertBeforeId, inputId, lineClass, suggestId, scrollId) {
  var self = this;
  self.win = win;
  self.doc = win.document;
  self.insertBefore = self.doc.getElementById(insertBeforeId);
  self.input = self.doc.getElementById(inputId);
  self.scroll = self.doc.getElementById(scrollId);
  ...
  self.createLine = function(klass) {
    var line = self.doc.createElement("pre");
  ...
  self.showSuggest = function(text) {
    //self.suggest.search();
    ...
    if (resultList.length != 0) {
      self.suggest.createSuggestArea(resultList);
      self.suggest.keyEventMove(self.Key.DOWN);
    }
  };
  ...
  self.SPKey = {};
  self.SPKey[self.Key.RETURN] = self.keyReturn;
  self.SPKey[self.Key.SPACE] = self.keySpace;
  self.SPKey[self.Key.UP] = self.keyUp;
  self.SPKey[self.Key.DOWN] = self.keyDown;
  self.SPKey[self.Key.DOT] = self.keyDot;
